...
class MultiScaleTCNBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.dropout(self.relu(self.bn(y)))
        return y + self.res(x)
...
class Model_Pure_TCN(nn.Module):
    ...
    def forward(self, x):
        ...
        w = torch.softmax(self.attn(torch.tanh(x)), dim=1)  # (B, T, 1)
        x = (x * w).sum(dim=1)  # (B, C)
...
class Model_TCN_SNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # time-major for spikingjelly
        x_t = x.permute(2, 0, 1)  # (T, B, C)
        spk_list = []
        for t in range(x_t.size(0)):
            spk_list.append(self.neuron(x_t[t]))
        spk = torch.stack(spk_list, dim=0)  # (T, B, C)
        spk = spk.permute(1, 2, 0)  # (B, C, T)

        # attention over time
        x_attn = spk.transpose(1, 2)  # (B, T, C)
        w = torch.softmax(self.attn(torch.tanh(x_attn)), dim=1)  # (B, T, 1)
        x_pool = (x_attn * w).sum(dim=1)  # (B, C)
